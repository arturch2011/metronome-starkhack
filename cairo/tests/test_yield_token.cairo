...
// use yield_token::tests::test_utils::{assert_eq};
...
use cairo::erc20::yield_token_erc20::YieldTokenERC20;
use cairo::erc20::yield_token_erc20::YieldTokenERC20::{Event::ERC20Event};
use cairo::erc20::erc20::{IERC20Dispatcher, IERC20DispatcherTrait};

use cairo::yield_token::{IYieldTokenDispatcher, IYieldTokenDispatcherTrait};
use cairo::yield_token::TransferRequest;


const INITIAL_SUPPLY: u256 = 1000000000;

fn setup() -> (ContractAddress, ContractAddress) {
    let erc20_class_hash = declare("YieldTokenERC20").unwrap();
    ...
    let yield_token_class_hash = declare("YieldToken").unwrap();
    // let account: ContractAddress = get_contract_address();

    let mut calldata = ArrayTrait::new();

    let (yield_token_address, _) = yield_token_class_hash.deploy(@calldata).unwrap();

    (erc20_address, yield_token_address)
}

#[test]
fn test_single_send() {
    let (erc20_address, yield_token_address) = setup();
    ...
    erc20.approve(yield_token_address, transfer_value * 2);

    assert(
        erc20.allowance(account, yield_token_address) == transfer_value * 2, 'Allowance not set'
    );
    ...
    let yield_token = IYieldTokenDispatcher { contract_address: yield_token_address };
    ...
    start_prank(CheatTarget::One(yield_token_address), account);
    yield_token.multisend(erc20_address, transfer_list);
